...
if __name__ == "__main__":
    px = (8, 2, 4, 1)
    x_list = (1, 3, 9, 10, 13, 5, 15, 11, 16, 14, 8, 7, 4, 12, 2, 6)
    ys0 = (15, 9, 8, 0, 0, 5, 12, 9, 9, 11, 10, 0, 8, 7, 2, 6)
    corrupted_ys0 = (15, 8, 8, 0, 0, 5, 12, 9, 9, 13, 10, 0, 8, 7, 2, 6)

    mod = 17
    pf = PrimeField(mod)

    aa = pf.eval_poly_at(px, 1)
    print(pf.eval_poly_at(px, 3))
    print(pf.eval_poly_at(px, 9))
    print(pf.eval_poly_at(px, 14))
    print(pf.mul(14, 14))

    bb = pf.eval_poly_at(px, 16)
    print(bb)
    ...
    ll = [pf.eval_poly_at(px, i) for i in x_list]
    print(ll)

    case = 0
    for i in range(0, 16):
        for j in range(0, 16):
            g = pf.div(pf.add(i, j), 2)
            h = pf.div(pf.sub(i, j), 6)
            if pf.add(g, h) == 4:
                print("res: " + str(i) + "," + str(j) + "//" + str(pf.add(i, j)) + "," + str(pf.sub(i,j)))
                case += 1
    print("case: ", case)

    # check if corrupted_ys0 is of degree 3 (3+1=4)
    xs0_parts = (4, 12, 2, 6)
    ys0_parts = (8, 7, 2, 6)  # this is common to voth ys0 and corrupted_ys0

    poly = pf.lagrange_interp(xs0_parts, ys0_parts)
    print(poly)

    # 1. check orignal ys0 first
    for i, v in enumerate(x_list):
        y = pf.eval_poly_at(poly, v)
        if ys0[i] != y:
            print("original ys0 incorrect")  # if everything works as I expected, it will never reach here.
            break

    # 2. check corrupted ys0
    for i, v in enumerate(x_list):
        y = pf.eval_poly_at(poly, v)
        if corrupted_ys0[i] != y:
            print("corrupted ys0 incorrect")  # if everything works as I expected, it will always fall in this line-!
            break
'''